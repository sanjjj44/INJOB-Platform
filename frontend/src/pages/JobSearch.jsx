...
const JobSearch = () => {
  ...
  const handleMatch = async () => {
    ...
    try {
      ...
      formData.append('mode', 'job')
      const res = await axios.post('http://127.0.0.1:8000/api/live-match', formData)
      setJobs(res.data)
    } catch (err) {
      alert('Error fetching jobs. Make sure backend is running!')
    }
    setMatching(false)
  }
      ...
      <div style={{ maxWidth: '1100px', margin: '0 auto', padding: '0 24px', position: 'relative', zIndex: 1 }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: '48px' }}>
          <h1 style={{ fontSize: '3rem', fontWeight: 900, color: '#fff', marginBottom: '8px' }}>
            JOB <span style={{ color: '#00ff88' }}>SEARCH</span>
          </h1>
        ...
        <div style={{
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 280px', gap: '24px', marginBottom: '24px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              ...
              <div style={{
                ...
                <p style={{ color: '#555', fontSize: '11px', marginBottom: '12px', letterSpacing: '1px' }}>SYSTEM STATUS</p>
                {[
                  { label: 'AI Engine', status: 'ONLINE' },
                  { label: 'Job API', status: 'ACTIVE' },
                  { label: 'NLP Model', status: 'READY' },
                ].map((s, i) => (
                  <div key={i} style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
                    <span style={{ color: '#666', fontSize: '12px' }}>{s.label}</span>
                    <span style={{ color: '#00ff88', fontSize: '11px', fontWeight: 700 }}>● {s.status}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          {uploadDone && (
            ...
            >
              {matching ? '🔍 FINDING NEURAL MATCHES...' : '⚡ UPDATE NEURAL MATCHES'}
            </button>
          )}
        </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '20px' }}>
              {jobs.map((job, i) => (
                <div key={i} style={{
                  ...
                  borderRadius: '20px',
                  padding: '24px',
                  boxShadow: `0 0 20px ${getScoreColor(job.score)}08`
                }}>
        ...
        {!uploadDone && (
          <div style={{
            ...
            <p style={{ color: '#555', fontSize: '14px' }}>
              Initialize a vector scan by uploading your resume to reveal potential career deployments.
            </p>
          </div>
        )}
      </div>
